LiveHelper = (function () {
    ...
    LiveHelper.canInsert = function (objRow, rawCriteria, joinedRow) {
        var canInsert = true;
        MysqlWrapper.Helper.forEachKey(rawCriteria, function (key) {
            if (!canInsert) {
                return;
            }
            try {
                if (objRow[key] !== undefined) {
                    ...
                    if (_valSplited[0] === "IN(") {
                        ...
                        var _symbolCombarison = MysqlWrapper.ConditionalConverter.toJS(_valSplited[0]);
                        var valComparison = _valSplited[1];
                        var ifEvalStatementStr = objRow[key] + _symbolCombarison + valComparison;
                        ifEvalStatementStr = MysqlWrapper.ConditionalConverter.toJSConditional(ifEvalStatementStr);
        ...
        criteria.tables.forEach(function (_tb) {
            ...
            joinedTableObj.on("INSERT", function (rows) {
                rows.forEach(function (row) {
                    ...
                    collectionArray.forEach(function (_objInlist) {
                        var joinedCriteria = {};
                        MysqlWrapper.Helper.forEachKey(joinedTableCriteria.rawCriteriaObject, function (key) {
                            try {
                        ...
                        if (canInsert) {
                            ...
                            var primkey = MysqlWrapper.Helper.toObjectProperty(table.primaryKey);
                            var objToFind = {};
            ...
            joinedTableObj.on("DELETE", function (rows) {
                rows.forEach(function (row) {
                    ...
                    toBeRemovedCriteria[MysqlWrapper.Helper.toObjectProperty(joinedTableObj.primaryKey)] = row[joinedTableObj.primaryKey];
                    var isArray = false;
            ...
            joinedTableObj.on("UPDATE", function (rows) {
                rows.forEach(function (row) {
                    ...
                    toBeUpdatedCriteria[MysqlWrapper.Helper.toObjectProperty(joinedTableObj.primaryKey)] = row[joinedTableObj.primaryKey];
                    var isArray = false;
                    if (joinedTableCriteria === undefined || joinedTableCriteria.queryRules === undefined ||
                        (joinedTableCriteria.queryRules !== undefined && ((joinedTableCriteria.queryRules.limitEnd - joinedTableCriteria.queryRules.limitStart) !== 1))) {
                        isArray = true;
